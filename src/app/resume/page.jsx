...
const Resume = () => {
  ...
  return (
    <div className='bg-gray-900 min-h-screen flex flex-col'>
      <Navbar />
      <div className="container mx-auto mt-10 p-6 bg-gray-200 rounded-lg shadow-lg w-full max-w-7xl min-h-screen overflow-auto">
        <h1 className="text-3xl font-semibold mb-12 text-gray-800">Upload Your Resume</h1>
 ...
 >
    ...
    <label htmlFor="fileInput" className="cursor-pointer">
       <div className="upload-content flex flex-col items-center">
        ...
        {/* Job role input */}
        <div className="mt-8">
          <label htmlFor="jobRole" className="block text-lg font-semibold text-gray-800 mb-2">Job Role:</label>
          <input
            type="text"
            id="jobRole"
            value={jobRole}
        ...
        {uploadedFileName && (
          <div className="mt-8 flex items-center justify-center bg-gray-800 rounded-2xl border-2 p-4">
...
{analysisResult && (
  <div className="mt-24">
    <h2 className="text-4xl mb-20  font-bold text-gray-900 text-center border-y pt-5 pb-6 border-black">Resume Analysis Report</h2>

    {/* Grid layout for scores */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      {Object.entries(parseAnalysisResult(analysisResult).scores).map(([title, value]) => (
        <div key={title} className="bg-white text-black p-6 rounded-lg shadow-md">
          <h3 className="text-lg font-semibold mb-2">{title}</h3>
          <p className="text- text-gray-900">{value}</p>
        </div>
      ))}
    </div>
    ...
    {parseAnalysisResult(analysisResult).additionalNotes.length > 0 && (
      <div className="bg-white p-6 rounded-xl shadow-lg">
        ...
        <p className="text-gray-700">{parseAnalysisResult(analysisResult).additionalNotes.join(' ')}</p>
      </div>
    )}
  </div>
)}
